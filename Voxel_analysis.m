...
for i=1:length(subpath)
    ...
    [pval_reordered_v1, fcn_reordered_v1] = corrcoef(tc_reordered);
...
for j = 1:length(subloc)
    ...
    sub_fcn(:,92:end) = [];
    sub_fcn(92:end,:) = [];
    fcn_vis = getfc(sub_fcn,1,9);
    fcn_aud = getfc(sub_fcn,10,11);
    fcn_lang = getfc(sub_fcn,12,31);
    fcn_mem = getfc(sub_fcn,32,49);
    fcn_sub = getfc(sub_fcn,50,58);
    fcn_cer = getfc(sub_fcn,59,70);
    fcn_senmot = getfc(sub_fcn,71,75);
    fcn_ins = getfc(sub_fcn,76,85);
    fcn_pre = getfc(sub_fcn,86,91);
...
figure;
gscatter(mlr_model_regressed.X(:,1),mlr_model_regressed.Y(:,6),mlr_model_regressed.X(:,2));
title('Cerebellar RSN');
ylabel('Mean FC value');
xlabel('Fluency ratio');
hold on;
plot(mlr_model_regressed.X(1:51,1),mlr_model_regressed.Yfitted(1:51,6),'-r','LineWidth',1);
legend('off');
plot(mlr_model_regressed.X(52:end,1),mlr_model_regressed.Yfitted(52:end,6),'-c','LineWidth',1);
legend('off');
legend({'Normal Cognition', 'MCI'}, 'Location','southoutside');
hold off;
% For language group
