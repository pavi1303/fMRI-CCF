...
niftiwrite(mask,'MNI_152_mask_c1c2c3.nii');
%
c1 = niftiread('c1MNI152_T1_2mm.nii');
...
list = [8,13,16,21,23,24,27,28,29,37];
no = length(list);
location = 'E:\LRCBH\Results\1.PCA\Trial';
for i=1:no
    pat = list(i);
    file = fullfile(location, sprintf('vt_%d.mat', pat));
    data = load(file);
